...
tema = st.text_input("Qual o foco das vendas de hoje?", placeholder="Ex: Oferta Relâmpago")
...
if st.session_state.roteiro_lista:
    ...
    for i, story_txt in enumerate(st.session_state.roteiro_lista):
        ...
        with st.expander(f"📌 {titulo}"):
            st.markdown(f'<div class="label-expert">📸 CENA:</div>', unsafe_allow_html=True)
            st.write(cena)
            st.markdown(f'<div class="label-expert" style="margin-top:10px;">🎙️ SCRIPT:</div>', unsafe_allow_html=True)
