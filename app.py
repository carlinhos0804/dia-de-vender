...
# 3. Gerenciamento do Timer (Memória do App)
...
# 5. Interface
tema = st.text_input("Qual o tema de hoje?", placeholder="Ex: Bastidores da nova coleção")
estilo = st.selectbox("Personalidade", ["Venda Direta", "Autoridade", "Humanizado", "Dicas"])

# Lógica do Botão com Contagem Regressiva
if can_generate():
    if st.button("🚀 GERAR ESTRATÉGIA AGORA"):
        if tema:
            with st.spinner('IA gerando cronograma...'):
                try:
                    prompt = f"Crie 5 stories para Instagram sobre: {tema}. Estilo: {estilo}. Retorne APENAS um JSON: [{{'horario': '...', 'cena': '...', 'jeito': '...', 'fala': '...'}}]"
                    ...
                    st.session_state.last_run = datetime.now() # Salva o horário que rodou
                    ...
                    for s in stories:
                        ...
                        """, unsafe_allow_html=True)
                    st.rerun() # Recarrega para ativar o timer
                except Exception as e:
                    st.error(f"Erro técnico. Tente novamente. ({e})")
        else:
    ...
    st.button(f"⏳ AGUARDE {remaining}s PARA GERAR NOVAMENTE", disabled=True)
    time.sleep(1)
    st.rerun() # Atualiza o contador na tela
